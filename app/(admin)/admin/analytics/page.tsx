...
export default async function AdminAnalyticsPage() {
  ...
  return (
    <div>
      <h1 className="font-syne text-2xl font-bold text-white mb-8">Аналитик</h1>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-[#111827] border border-white/10 rounded-2xl p-6">
          <h2 className="font-syne text-lg font-bold text-white mb-4">Сүүлийн төлбөрүүд</h2>
          <div className="space-y-3">
            {recentPayments.map((p) => (
              <div key={p.id} className="flex items-center justify-between py-2 border-b border-white/5">
                <div>
                  <div className="text-sm text-white">{p.user.name}</div>
                  <div className="text-xs text-gray-400">{p.course.titleMn}</div>
                </div>
                <div className="text-sm text-green-400 font-medium">₮{p.amount.toLocaleString()}</div>
              </div>
            ))}
            {recentPayments.length === 0 && <p className="text-gray-500 text-sm">Төлбөр байхгүй</p>}
          </div>
        </div>

        <div className="bg-[#111827] border border-white/10 rounded-2xl p-6">
          <h2 className="font-syne text-lg font-bold text-white mb-4">Эрэлттэй курсууд</h2>
          <div className="space-y-3">
            {topCourses.map((c, i) => (
              <div key={c.titleMn} className="flex items-center justify-between py-2 border-b border-white/5">
                <div className="flex items-center gap-3">
                  <span className="text-gray-500 text-sm w-4">{i + 1}</span>
                  <div className="text-sm text-white">{c.titleMn}</div>
                </div>
                <div className="text-xs text-gray-400">{c.totalStudents} оюутан</div>
              </div>
            ))}
            {topCourses.length === 0 && <p className="text-gray-500 text-sm">Курс байхгүй</p>}
          </div>
        </div>
      </div>
    </div>
