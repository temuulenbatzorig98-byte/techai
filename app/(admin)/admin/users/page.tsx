...
export default async function AdminUsersPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <h1 className="font-syne text-2xl font-bold text-white mb-8">
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="border-b border-white/5 hover:bg-white/2">
                ...
                <td className="px-4 py-3 text-gray-400">{u.email || u.phone || '—'}</td>
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${u.role === 'ADMIN' ? 'bg-purple-600/20 text-purple-400' : 'bg-gray-600/20 text-gray-400'}`}>
                    {u.role}
                  </span>
                </td>
                <td className="px-4 py-3 text-gray-300">{u._count.enrollments}</td>
