export function StatsBar() {
  ...
  return (
    <section className="py-8 border-y border-slate-200 dark:border-white/10 bg-white/2">
      <div className="max-w-4xl mx-auto px-4 grid grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map(({ value, label }) => (
          <div key={label} className="text-center">
            ...
            <div className="text-sm text-slate-500 dark:text-gray-400 mt-1">{label}</div>
          </div>
        ))}
      </div>
    </section>
